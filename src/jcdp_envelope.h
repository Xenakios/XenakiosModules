...
public:
    ...
    breakpoint_envelope(std::string name, double minv=0.0, double maxv=1.0)
	...
	{
		std::unique_ptr<breakpoint_envelope> result(new breakpoint_envelope);
	...
	*/
	...
	void adjustEnvelopeSegmentValues(int index, double amount)
	{
		...
		{
			m_nodes.back().pt_y = clamp(float(m_old_nodes.back().pt_y+amount),0.0f,1.0f);
			return;
		}
	...
	inline double getTransformedValue(double x)
	{
		...
		if (m_transform_y_sinus>0.0)
			shifted+=m_transform_y_sinus * sin(2*3.141592653*(x-m_transform_x_shift)*m_transform_y_sinus_freq);
